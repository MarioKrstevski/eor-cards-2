...
def run_segment(section_data: dict, sections: list[dict], model: str):
    ...
    user = (
        ...
        "1. ONE IDEA PER UNIT. If a single sentence or line covers two distinct exam "
        "facts — e.g. a normal value AND what its abnormality means, or a gestational "
        "limit AND the anesthesia used, or a method AND its indication — split it into "
        "SEPARATE standalone units, one idea each. Lean toward splitting whenever there "
        "is a genuine second idea.\n"
        "2. STRUCTURE decides sibling vs bundle. Make a sibling_set ONLY when the items "
        "appear on their OWN separate lines/bullets/rows, each with its own explanation "
        "(e.g. 'Expectant management: … / Medical management: … / Surgical management: …', "
        "or methods/causes each described on its own line). Then each member is its own "
        "card with the others in the footer.\n"
        "3. LAYERED LISTS. For a list of items inside a SINGLE flowing prose sentence: "
        "ALWAYS emit one combined standalone unit whose card lists ALL the items together "
        "(even with brief parentheticals like '(≥35 years)'). THEN, if the items fall "
        "into natural sub-groups (e.g. age-related, substance use, chronic diseases), "
        "ALSO emit one extra standalone unit per group testing just that group's items. "
        "Do this whenever real grouping is possible — an over-generated card is cheap to "
        "delete, a missing card is expensive to make by hand — but never invent a group "
        "that isn't there, and NEVER make a single-item card (a lone 'smoking' is not a "
        "card; a group 'substance use: smoking, alcohol, cocaine' is). Example: the risk-"
        "factor sentence yields a combined 'all risk factors' card PLUS an age card, a "
        "substance-use card, and a chronic-disease card.\n"
        "4. The Sequential/Diagnostic exclusion still makes workup steps standalone.\n"
        "WHEN IN DOUBT BETWEEN FEWER AND MORE CARDS, PREFER MORE — as long as each extra "
        "card tests a real, distinct idea or a real sub-group. Over-generation is fine "
        "(easy to delete); fabricating content that isn't in the source is not.\n"
...
def run_author(section_data: dict, units: list[dict], sections: list[dict], model: str):
    ...
    user = (
        ...
        "answer that unit's QUESTION. Keep the anchor visible by naming it NATURALLY "
        "WITHIN the sentence (e.g. 'Risk factors for spontaneous abortion include …'), "
        "and cloze only the answer. Do NOT prepend a label prefix: never start a card "
        "with '<Topic>:' or '<Topic> risk factors:' when the sentence already states the "
        "subject — that prefix is redundant and must be omitted. For sibling-set members, "
        "the stem must contain "
