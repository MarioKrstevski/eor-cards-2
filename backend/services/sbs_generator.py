...
def run_segment(client, section_data: dict, sections: list[dict], model: str):
    ...
    user = (
        ...
        "Segment this section into units. Decide which are standalone vs sibling sets "
        "using the rules. For each unit, list its members with the VERBATIM source text "
        "for each. Do NOT write any cards yet — return only the plan via submit_plan."
    )
