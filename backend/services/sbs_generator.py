...
def run_segment(client, section_data: dict, sections: list[dict], model: str):
    ...
    user = (
        ...
        "1. A sentence that contains several DISTINCT testable facts (each a different "
        "idea) becomes SEVERAL standalone units — one idea each.\n"
        ...
        "3. Items listed INSIDE A SINGLE flowing prose sentence are ONE standalone unit — "
        "the card lists them together — even if some carry a brief parenthetical clarifier "
        "like '(≥35 years)'. Do NOT split one sentence's inline items into separate cards. "
        "Example: 'Risk factors include advanced maternal age (≥35 years), paternal age "
        "≥45, smoking, alcohol or cocaine use, and poorly controlled chronic diseases' is "
        "ONE unit (one card that lists the risk factors), NOT a sibling set and NOT a "
        "smoking-only card.\n"
